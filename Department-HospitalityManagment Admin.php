...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_research'])) {
  ...
  $sql = "INSERT INTO  hospitality_research (research_date, research_title, author, location, status) 
          VALUES (?, ?, ?, ?, ?)";

  if ($stmt = $conn->prepare($sql)) {
      $stmt->bind_param("sssss", $researchDate, $researchTitle, $author, $location, $status);
      if ($stmt->execute()) {
          echo "<script>alert('Research added successfully!'); window.location.href='Department-HospitalityManagment Admin.php';</script>";
      } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_research'])) {
  ...
  $sql = "UPDATE  hospitality_research 
  ...
  if ($stmt !== false) {
      ...
      if ($stmt->execute()) {
          echo "<script>alert('Research updated successfully!'); window.location.href='Department-HospitalityManagment Admin.php';</script>";
      } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_research'])) {
  ...
  $sql = "DELETE FROM  hospitality_research WHERE id = ?";
